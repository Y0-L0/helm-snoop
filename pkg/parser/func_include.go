...
func includeFn(ctx *evalCtx, call Call) evalResult {
	...
	if len(call.Args) < 1 {
		...
		assert.Must("include: requires at least 1 argument")
		return evalResult{}
	}
	...
	if len(nameResult.args) == 0 {
		...
		assert.Must("include: template name must be a string literal")
		return evalResult{}
	}
	...
	if ctx.idx == nil {
		...
		assert.Must("include: template index not available")
		return evalResult{}
	}
	...
	if !found {
		...
		assert.Must(fmt.Sprintf("include: template %q not found", templateName))
		return evalResult{}
	}
